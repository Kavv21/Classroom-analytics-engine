...
/**
 * The per-question answering UI — one question at a time, debounced
 * autosave, offline retry, local draft recovery.
 *
 * CSV upload (the parent route) is now the primary path, but this remains
 * the route for a student who wants to work through the questions
 * incrementally, and it is the surface the no-auto-submit guarantees are
 * tested against (tests/unit/no-auto-submit.test.tsx). Same attempt, same
 * RPCs — a student can start here and finish by upload, or the reverse.
 */
export default async function TakeAssignmentQuestionsPage({
  params,
}: {
