...
/**
 * Alluvial (flow) data for one mapping_transition_summary row. Labels are
 * neutral response values — never "improved"/"correct"/"learned"
 * (.claude/rules/analytics.md). Missing buckets flow to explicit
 * missing-data nodes so they are visible but never mixed into S-states;
 * the summary's missing_a2_from_0/1 and missing_a1_to_0/1 splits keep the
 * answered side's value exact.
 */
export function alluvialFromTransitionCounts(counts: {
  s00: number;
  s01: number;
  s10: number;
  s11: number;
  missingA2From0: number;
  missingA2From1: number;
  missingA1To0: number;
  missingA1To1: number;
  missingBoth: number;
}): AlluvialData {
  const nodes = [
    { id: "a1:0", label: "Assignment 1 — 0" },
    { id: "a1:1", label: "Assignment 1 — 1" },
    { id: "a1:missing", label: "Assignment 1 — no answer" },
    { id: "a2:0", label: "Assignment 2 — 0" },
    { id: "a2:1", label: "Assignment 2 — 1" },
    { id: "a2:missing", label: "Assignment 2 — no answer" },
  ];
  const links = [
    { source: "a1:0", target: "a2:0", value: counts.s00 },
    { source: "a1:0", target: "a2:1", value: counts.s01 },
    { source: "a1:1", target: "a2:0", value: counts.s10 },
    { source: "a1:1", target: "a2:1", value: counts.s11 },
    { source: "a1:0", target: "a2:missing", value: counts.missingA2From0 },
    { source: "a1:1", target: "a2:missing", value: counts.missingA2From1 },
    { source: "a1:missing", target: "a2:0", value: counts.missingA1To0 },
    { source: "a1:missing", target: "a2:1", value: counts.missingA1To1 },
    { source: "a1:missing", target: "a2:missing", value: counts.missingBoth },
  ].filter((l) => l.value > 0);
